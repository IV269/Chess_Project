...
class _Board {
    ...
    canMove(x, y, x2, y2, tomove = true, tocheck = false) {
                ...
                if (Math.abs(dx) <= 1 && Math.abs(dy) <= 1 && !(dx == 0 && dy == 0)) {
                    ...
                    if (x2 == 0) {
                        if (this.color == "w") {
                            ...
                            conpos = this.RokirB[0];
                        }
                    } else {
                        if (this.color == "w") {
                            conpos = this.RokirW[1];
                        } else {
                    ...
                    if (conpos) {
                        let xmin = Math.min(x, x2),
                            xmax = Math.max(x, x2);
                        let config = true;
                        for (let i = xmin + 1; i < xmax && config; i++) {
                            if (this.board[y][i][1] != undefined) {
                                config = false;
                            }
                        }
        ...
        if (tomove && ans && !concheck) {
            ...
            // Checking on posibilities for castling
            if (fig.name == "K") {
                if (fig.color == "w") {
                    this.RokirW = [false, false];
                } else {
                    this.RokirB = [false, false];
                }
            } else if (fig.name == "R") {
                if (fig.color == "w") {
